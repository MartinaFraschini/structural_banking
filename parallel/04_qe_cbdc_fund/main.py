...
output_path = './res/rF-' + str(rF_type) + '_gamma-' + str(int(gamma_num*100))
...
fund_mat = np.zeros(NrD)                                # depends on rD(t)
        ...
        for rD_ind in range(NrD):
            ...
            if okdep:
                ...
                if delta*dep_mat[rD_ind] - tran_mat[rD_ind] > phi:
                    ...
                    res_mat[rD_ind] = phi
                    fund_mat[rD_ind] = phi - delta*dep_mat[rD_ind] + tran_mat[rD_ind]
            eq_mat[rL_ind, rD_ind, zm_ind] = Lopt + res_mat[rD_ind] - dep_mat[rD_ind] - fund_mat[rD_ind]
            for z_ind in range(Nz):
                ...
                profit_mat[rL_ind, rD_ind, zm_ind, z_ind] = Profit(Ropt, z_grid[z_ind], rL, Lopt, dep_mat[rD_ind], 
                                                                   res_mat[rD_ind], fund_mat[rD_ind], rD, rM, rF, 
                                                                   lam, a, b, sigmaeps)
        okdep = 0
...
for t in range(1, Tsim-1):
    ...
    CB_fund[t] = fund_mat[int_rates_dep_ind[t]]
    CB_assets[t] = reserves[t] + cbdc[t] - CB_fund[t]
    equity[t] = loans[t] + reserves[t] - deposits[t] - CB_fund[t]
    if profits[t] > 0:
